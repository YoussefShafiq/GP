                ...
                <tbody>
                    {tasks.length > 0 ? <>
                        {tasks.map((task, rowIndex) => (
                            <tr onClick={() => {
                                if (!location.pathname.includes(`/task-details`)) {
                                    setselectedTask(task); // Set the selected task
                                    navigate(`/task-details/${task.id}`); // Navigate to the task details page
                                }
                            }}
                                key={rowIndex}
                                className={`bg-white border-b cursor-pointer dark:bg-dark1 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-opacity-80 duration-150  `}
                            >
