...
export default function Login() {
    ...
    async function login(values) {
        ...
        try {
            setError('')
            let { data } = await axios.post('http://brainmate.ct.ws/api/v1/auth/login', values)
            setloading(false)
            console.log(data);
    ...
    async function googleToken() {
        if (paramToken) {
            ...
            try {
                ...
                let { data } = await axios.post('http://brainmate.ct.ws/api/v1/auth/validate-token', {
                    ...
                    {thirdpartyAuthenticationLoading ? <>
                        ...
                        <div className="w-full p-5 lg:p-0 lg:w-1/2 flex flex-col items-center overflow-y-scroll" style={{ scrollbarWidth: 'none' }}>
                            ...
                            <div className='w-full max-w-sm my-5' >
                                <button onClick={() => { window.location.href = 'http://brainmate.ct.ws/api/v1/auth/auth/google' }} className='w-full h-12 rounded-xl text-primary dark:text-gray-500 text-opacity-80 border border-solid border-primary border-opacity-40 flex justify-center items-center space-x-3 ' > <img className='h-2/3 me-2' src={googleLogo} alt="google login" /> Login with google</button>
                            </div>
